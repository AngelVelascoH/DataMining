...
dfTraining = pd.read_csv("PPHTraining.csv")
dfTesting = pd.read_csv("PPHTesting.csv")
...
for element in res1:
    ...
    print('Predicted Value: ', element, ' Real value: ',
          y_testing[index], " % Error: ", error)
    index = index+1
...
with open(pathCsvfile, 'w', encoding='utf-8', newline='') as csvFile:
    ...
    for element in res1:
        ...
        writer.writerow({'predictedValue': "'"+format(element)+"'", 'RealValue': "'" +
                        format(y_testing[index])+"'", 'ErrorPercent': "'"+format(error)+"'"})
        index = index+1
print("---------------------------------------------")


print("--------------------SVR Radial bases-------------------------")
